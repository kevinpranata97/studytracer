...
export default {
  ...
  async created () {
    ...
    this.jawab.npm = userdata.profile.npm
    this.$axios.get('/alumni/viewkuisioner', { headers: {
  ...
  methods: {
    edit (no, text, category1) {
      ...
      this.kuis.text = text
      let token = this.$q.localStorage.getItem('datauser')
      this.$axios.post('/alumni/viewkuisionergroup', { category: this.kuis.category })
        .then(res1 => {
          this.indikator = res1.data.results.data
          this.$axios.post('/alumni/selectkuisioner', this.jawab, { headers: {
            'x-access-token': token
          } })
            .then(res => {
    ...
    onsubmit () {
      ...
      if (this.editkuis === true) {
        this.$axios.post('/alumni/editkuisioner', this.jawab, { headers: {
          'x-access-token': token
        } })
          .then(res => {
            this.$q.notify({
        ...
        this.$axios.post('/alumni/inputdatakuisioner', this.jawab, { headers: {
          'x-access-token': token
        } })
          .then(res => {
            this.$q.notify({
